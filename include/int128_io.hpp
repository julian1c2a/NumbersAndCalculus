/**
 * @file int128_io.hpp
 * @brief Operadores de I/O y conversión de strings para __int128
 ...
 * @return String representando el valor
 */
inline std::string to_string(unsigned __int128 value) {
  std::ostringstream oss;
  oss << value;
  return oss.str();
}
 ...
 */
[[deprecated("Use to_string() instead")]]
inline char *to_cstr(unsigned __int128 value) {
  std::string str = to_string(value);
  char *result = new char[str.length() + 1];
  std::strcpy(result, str.c_str());
  return result;
 ...
 */
inline __int128 from_string_i128(const std::string &str) {
  if (str.empty()) {
 ...
 * __int128
 */
inline unsigned __int128 from_string_ui128(const std::string &str) {
  if (str.empty()) {
 ...
 * @throws std::out_of_range Si el número está fuera del rango de __int128
 */
inline __int128 from_cstr_i128(const char *cstr) {
  if (cstr == nullptr) {
 ...
 * __int128
 */
inline unsigned __int128 from_cstr_ui128(const char *cstr) {
  if (cstr == nullptr) {
    throw std::invalid_argument(
...
int main() {
  ...
  try {
    ...
    // Prueba con stream de salida
    std::cout << "Usando operator<<: " << big_number << std::endl;

    // Prueba con stringstream
    std::stringstream ss;
    ss << big_number;
    ...
    // Prueba con C-strings (deprecated)
    char *cstr = to_cstr(big_number);
    std::cout << "C-string: " << cstr << std::endl;
    delete[] cstr;

    __int128 from_cstr = from_cstr_i128("987654321098765432109876543210");
    std::cout << "Desde C-string: " << from_cstr << std::endl;
